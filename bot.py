...
def handle_start(message):
    ...
    welcome_text = (
        "Добро пожаловать! Я анализирую состав продуктов по фото этикетки.\n"
        "\n"
        "Как получить точный результат:\n"
        "\n"
        "📸 Фотографируйте именно состав — строку «Состав:» на обороте упаковки, "
        "не лицевую сторону и не штрихкод\n"
        "\n"
        "🔍 Кадрируйте плотно — текст состава должен занимать большую часть кадра. "
        "Если этикетка маленькая, поднесите камеру вплотную\n"
        "\n"
        "💡 Убедитесь, что нет бликов и теней — текст должен читаться чётко\n"
        "\n"
        "📎 Отправляйте как файл (через скрепку → «Файл»), а не обычное фото — "
        "Telegram не сжимает файлы, качество выше\n"
        "\n"
        "Мы не храним ваши фото и не используем их для обучения моделей.\n"
        "\n"
        "Пришлите фото — и начнём."
    )
...
def handle_help(message):
    ...
    help_text = (
        ...
        "\n"
        "Совет по съёмке:\n"
        "Кадрируйте именно строку «Состав:» — плотно, без лишних полей. "
        "Отправляйте как файл (📎) для лучшего качества распознавания.\n"
        "\n"
        "Команды:\n"
        "/start — инструкция и начало работы\n"
        "/help — эта справка"
    )
